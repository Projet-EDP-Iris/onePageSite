export const translations = {
  fr: {
    nav: {
      download: 'Télécharger',
      iris: 'Iris',
      pricing: 'Nos Tarifs',
      aboutUs: 'À propos',
      faq: 'FAQ',
    },
    hero: {
      headline: "Organiser votre quotidien en un clic",
      headlineBr: "Organiser votre quotidien en un clic",
      headlineBr2: "",
      subtext: "Iris gère vos emails en action tout en respectant votre vie privée",
      cta: 'TÉLÉCHARGER',
      subtextCta: 'Commencez gratuitement ou',
      bookDemo: 'réservez une démo',
    },
    video: {
      title: 'Comment fonctionne Iris ?',
      description: 'Iris transforme vos emails et calendriers en actions : elle crée des tâches, suggère des réponses et met en avant les messages prioritaires. Il ne vous reste plus qu’à valider en un clic. Regardez notre tutoriel ci-dessous.',
      iframeTitle: 'Vidéo tutoriel Iris',
    },
    trusted: {
      trustedBy: 'Des entreprises leaders choisissent',
      leading: ' Iris ',
      brands: 'pour simplifier la gestion de leurs emails et de leurs tâches',
    },
    allMajor: {
      title: 'Toutes les applications intégrées',
      downloadNow: 'Télécharger',
    },
    pricing: {
      title: 'Choisissez l\'offre qui vous ',
      scales: 'correspond',
      titleSuffix: '',
      subtitle: 'Commencez gratuitement et évoluez avec nous. Toutes les offres incluent nos fonctionnalités principales avec une garantie satisfait ou remboursé de 90 jours.',
      plans: [
        {
          plan: 'Gratuit',
          price: 0,
          originalPrice: null,
          isPopular: false,
          features: [
            '3 exécutions de calendrier par mois',
            'Intégration calendrier de base',
            'Notifications par e-mail',
            'Accès application mobile',
            'Support communautaire',
          ],
        },
        {
          plan: 'Pro',
          price: 29,
          originalPrice: 35,
          isPopular: true,
          features: [
            'Exécutions calendrier illimitées',
            'Planification IA avancée',
            'Toutes les intégrations',
            'Support prioritaire',
            'Workflows personnalisés',
            'Collaboration équipe (jusqu\'à 5)',
            'Tableau de bord analytique',
            'Export de rapports',
          ],
        },
        {
          plan: 'Entreprise',
          price: 99,
          originalPrice: 115,
          isPopular: false,
          features: [
            'Tout Pro inclus',
            'Membres d\'équipe illimités',
            'Account manager dédié',
            'Support premium 24/7',
            'Intégrations sur mesure',
            'SSO et sécurité avancée',
            'Garantie SLA',
            'Options white-label',
            'Accès API',
          ],
        },
      ],
    },
    about: {
      title: 'À propos de nous',
      theTeam: 'Une équipe dédiée à rendre votre quotidien plus simple et plus organisé',
      getInTouch: 'Des questions ? Écrivez-nous sur “Contactez-nous” ou retrouvez-nous sur LinkedIn',
      contactUs: 'Contactez-nous',
      linkedIn: 'LinkedIn', 
    },
    faq: {
      title: 'Encore des ',
      questions: 'questions ?',
      subtitle: 'Nous répondons à vos questions les plus fréquentes sur Iris.',
      items: [
        {
          question: 'Qu\'est-ce qu\'Iris ?',
          answer: "Là où une simple boîte mail s’arrête, Iris prend le relais, ce compagnon intelligent transforme vos emails en actions concrètes. Elle organise tes échanges par priorité et s’intègre aux applications que tu utilises déjà pour gérer ton agenda, tes tâches ou tes réponses en un seul clic.",
        },
        {
          question: 'Comment fonctionne Iris ?',
          answer: "Iris se connecte à tes mails et calendriers pour transformer automatiquement tes messages en actions, tout en gardant tes données privées et sécurisées sur ton ordinateur. Elle détecte les échéances, crée des tâches à suivre, suggère des réponses adaptées, propose des créneaux disponibles et met en avant les messages prioritaires pour faciliter le suivi. Il ne te reste plus qu’à valider ces actions en un seul clic. Tes informations restent entièrement sous ton contrôle et ne sont jamais envoyées sur des serveurs externes",
        },
        {
          question: 'À quelles applications Iris s’intègre-t-elle ?',
          answer: "Iris est une application de bureau qui centralise vos outils préférés. Elle s’intègre à Gmail et Outlook pour gérer vos e-mails, et se synchronise avec Google Calendar et Apple Calendar pour organiser vos rendez-vous au même endroit. La version bêta est en cours de déploiement, et vos données restent privées et protégées grâce à des standards stricts de sécurité.",
        },
        {
          question: 'Sur quelles plateformes Iris est-il disponible ?',
          answer: "Iris est disponible sur macOS et Windows. Elle vous permet de gérer vos mails, vos tâches et vos rendez-vous depuis votre ordinateur.",
        },
        {
          question: 'À qui s\'adresse Iris ?',
          answer: "À toi, si ta boîte mail est devenue une source de charge mentale plutôt qu’un outil. Iris est pensée pour les personnes qui reçoivent beaucoup d’e-mails qui demandent d’agir, répondre, planifier, suivre une demande ou ne pas oublier une relance. Si tu veux garder la main mise sur tes mails tout en dépensant ton temps efficacement, Iris transforme ce flux en un espace de travail clair et organisé.",
        },
        {
          question: 'Est-ce qu’Iris est payante ?',
          answer: "Non, tu peux commencer à utiliser Iris totalement gratuitement. La version gratuite te permet de tester le compagnon et de prendre le contrôle de tes e-mails et rendez-vous sans rien payer. Si tu as ensuite besoin de gérer un plus grand volume d’actions ou d’accéder à des fonctionnalités avancées (création et envoi de newsletters directement à tes destinataires), des abonnements Pro et des offres Entreprise sont proposés.",
        },
        {
          question: 'Iris respecte-t-elle la confidentialité et la RGPD ?',
          answer: "Oui, à 100%. L’intelligence artificielle d’Iris tourne localement, c’est-à-dire directement sur ton ordinateur. Tes e-mails et tes données ne sont jamais envoyés sur des serveurs externes pour être analysés. Ce qui se passe dans ta boîte mail reste dans ta boîte mail. De plus, Iris ne fait rien sans ton accord, elle te propose des actions, mais c’est toujours toi qui as le dernier mot.",
        },
        {
          question: 'En quoi Iris est-elle différente des autres outils ?',
          answer: "Iris ne se contente pas de trier tes e-mails, elle comprend ce qu’ils impliquent. Elle transforme tes messages en actions concrètes, relie automatiquement ta messagerie à ton agenda et organise tes tâches pour toi. Tout est préparé, il ne te reste plus qu’à valider en un clic, tout en gardant le contrôle total de tes données et de ton workflow.",
        },
      ],
    },
    footer: {
      tagline: 'Simplifiez la gestion de vos emails, rendez-vous et tâches. Iris vous aide à rester organisé et efficace. Vous ne trouvez pas votre réponse ? Contactez-nous :',
      linkedIn: 'LinkedIn',
      privacyPolicy: 'Politique de confidentialité',
      cookiePolicy: 'Politique des cookies',
      copyright: '© 2025-2026 Iris. Tous droits réservés.',
    },
    modal: {
      title: 'Aidez-nous à améliorer Iris',
      subtitle: 'Partagez vos idées et participez à l\'avenir de la productivité',
      skip: 'Plus tard',
      once: 'Cette fenêtre n\'apparaîtra qu\'une fois',
      loading: 'Chargement…',
      iframeTitle: 'Questionnaire Iris',
    },
    pricingCard: {
      mostPopular: 'Le plus populaire',
      perMonth: '/mois',
      save: 'Économisez',
      contactSales: 'Contacter les ventes',
      getStarted: 'Commencer',
      whatsIncluded: 'Inclus :',
      guarantee: 'Garantie satisfait ou remboursé 90 jours • Annulez à tout moment',
    },
    calendarCarousel: {
  sectionTitle: 'Nos fonctionnalités',
  cards: [
    { id: 1, image: '/image/category.jpeg', title: 'Tri intelligent des emails', description: 'Iris classe automatiquement vos emails en trois catégories pour garder une boîte de réception claire.' },
    { id: 2, image: '/image/resume.jpeg', title: 'Résumés des longs mails', description: 'Iris résume vos emails pour comprendre l’essentiel en quelques secondes.' },
    { id: 3, image: '/image/reponse.jpeg', title: 'Réponses et actions', description: 'Iris propose des réponses prêtes à envoyer et suggère les prochaines actions comme planifier une réunion.' },
  ],
},
  },
  en: {
    nav: {
      download: 'Download',
      iris: 'Iris',
      pricing: 'Pricing',
      aboutUs: 'About us',
      faq: 'FAQ',
    },
    hero: {
      headline: "Organize your day in one click",
      headlineBr: 'Organize your day in one click',
      headlineBr2: '',
      subtext: 'Iris turns your emails into actions while keeping your data private.',
      cta: 'DOWNLOAD',
      subtextCta: 'Start monitoring for free or ',
      bookDemo: 'book a demo',
    },
    video: {
      title: 'How Iris Works ?',
      description: 'Iris turns your emails and calendars into actions: it creates tasks, suggests replies, and highlights priority messages. All you have to do is approve them with one click. Watch our tutorial below.',
      iframeTitle: 'Iris Tutorial Video',
    },
    trusted: { trustedBy: 'Leading companies choose', leading: ' Iris ', brands: 'to simplify their email and task management' },
    allMajor: { title: 'All Major Apps Integrated', downloadNow: 'Download Now' },
    pricing: {
      title: 'Choose the plan that ',
      scales: 'scales',
      titleSuffix: ' with you',
      subtitle: 'Start free and upgrade as you grow. All plans include our core features with a 90-day money-back guarantee.',
      plans: [
        { plan: 'Free', price: 0, originalPrice: null, isPopular: false, features: ['3 calendar runs per month','Basic calendar integration','Email notifications','Mobile app access','Community support'] },
        { plan: 'Pro', price: 29, originalPrice: 35, isPopular: true, features: ['Unlimited calendar runs','Advanced AI scheduling','All app integrations','Priority support','Custom workflows','Team collaboration (up to 5)','Analytics dashboard','Export reports'] },
        { plan: 'Enterprise', price: 99, originalPrice: 115, isPopular: false, features: ['Everything in Pro','Unlimited team members','Dedicated account manager','24/7 premium support','Custom integrations','SSO & advanced security','SLA guarantee','White-label options','API access'] },
      ],
    },
    about: { title: 'About Us', theTeam: 'A team dedicated to making your daily life simpler and more organized', getInTouch: 'Questions? Write to us via “Contact Us” or find us on LinkedIn', contactUs: 'Contact us', linkedIn: 'LinkedIn' },
    faq: {
      title: 'Still have ',
      questions: 'questions?',
      subtitle: 'We answer your most frequently asked questions about Iris.',
      items: [
        { question: 'What is Iris?', answer: "Where a simple mailbox stops, Iris takes over. This intelligent companion transforms your emails into actionable tasks. It organizes your exchanges by priority and integrates with the apps you already use to manage your calendar, tasks, or responses in a single click." },
        { question: 'How does Iris work?', answer: "Iris connects to your emails and calendars to automatically turn messages into actions while keeping your data private and secure on your computer. It detects deadlines, creates follow-up tasks, suggests appropriate replies, offers available time slots, and highlights priority messages for easier tracking. You just need to approve these actions with a single click. Your information stays fully under your control and is never sent to external servers." },
        { question: 'What apps does Iris integrate with?', answer: "Iris is a desktop application that centralizes your favorite tools. It integrates with Gmail and Outlook to manage your emails, and syncs with Google Calendar and Apple Calendar to organize your appointments in one place. The beta version is being rolled out, and your data remains private and secure with strict security standards." },
        { question: 'Which platforms is Iris available on?', answer: "Iris is available on macOS and Windows. It allows you to manage your emails, tasks, and appointments directly from your computer." },
        { question: 'Who is Iris for?', answer: "For anyone whose mailbox has become a source of mental load rather than a tool. Iris is designed for people who receive many emails that require action, responses, planning, follow-ups, or reminders. If you want to keep control of your emails while using your time efficiently, Iris turns this flow into a clear, organized workspace." },
        { question: 'Is Iris paid?', answer: "No, you can start using Iris completely for free. The free version lets you test the assistant and control your emails and appointments at no cost. If you need more actions or advanced features (like newsletter creation and sending), Pro and Enterprise subscriptions are available." },
        { question: 'Does Iris respect privacy and GDPR?', answer: "Yes, 100%. Iris AI runs locally on your computer. Your emails and data are never sent to external servers. Everything stays on your device. Actions are only proposed—you have the final control." },
        { question: 'How is Iris different from other productivity tools?', answer: "Iris does not just sort your emails, it understands them. It transforms your messages into actionable tasks, automatically links your inbox to your calendar, and organizes your tasks for you. Everything is prepared, and you just approve with one click, keeping full control over your data and workflow." },
      ],
    },
    footer: {
      tagline: 'Simplify managing your emails, appointments, and tasks. Iris helps you stay organized and efficient. Can\'t find your answer? Contact us:',
      linkedIn: 'LinkedIn',
      privacyPolicy: 'Privacy Policy',
      cookiePolicy: 'Cookie Policy',
      copyright: '© 2025-2026 Iris. All rights reserved.',
    },
    modal: { title: 'Help Us Improve Iris', subtitle: 'Share your thoughts and help shape the future of productivity', skip: 'Skip for now', once: 'This will only appear once', loading: 'Loading…', iframeTitle: 'Iris Questionnaire' },
    pricingCard: { mostPopular: 'Most Popular', perMonth: '/mo', save: 'Save', contactSales: 'Contact Sales', getStarted: 'Get Started', whatsIncluded: "What's included:", guarantee: '90-day money-back guarantee • Cancel anytime' },
    calendarCarousel: {
  sectionTitle: 'Our Features',
  cards: [
    {
      id: 1,
      image: '/image/category.jpeg',
      title: 'Smart Email Sorting',
      description: 'Iris automatically sorts your emails into three categories to keep your inbox clear.'
    },
    {
      id: 2,
      image: '/image/resume.jpeg',
      title: 'Summaries of long emails',
      description: 'Iris summarizes your emails to grasp the essentials in seconds.'
    },
    {
      id: 3,
      image: '/image/reponse.jpeg',
      title: 'Replies and Actions',
      description: 'Iris offers ready-to-send replies and suggests the next actions, such as scheduling a meeting.'
    }
  ]
},
  },
};